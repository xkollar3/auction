...
public class RefundOrderCommandHandler {
  ...
  public void handle(RefundOrderCommand command) {
    ...
    String refundId = stripeFundsApiClient.refundPayment(command.getPaymentIntentId(), command.getOrderId());

    commandGateway.send(new FinishRefundCommand(command.getOrderId(), command.getPaymentIntentId()));
